...
`rpg --speech-backend local-command --speech-command '<command>'` starts a long-lived local speech process and sends the same `SpeechRequest` shape as newline-delimited JSON. The local process must ack each request promptly on stdout so the TUI can keep accepting input while audio work continues asynchronously; diagnostics belong on stderr. This is the `offline-voice-runtime` integration point and the first shared local speech foundation intended for later `esuna` / `osaka-kenpo` reuse.
