...
`rpg --speech-backend local-command --speech-command '<command>'` starts a long-lived local speech process and sends the same `SpeechRequest` shape as newline-delimited JSON. This is the `offline-voice-runtime` integration point and the first shared local speech foundation intended for later `esuna` / `osaka-kenpo` reuse.
...
The speech pipeline distinguishes at least four utterance intents: ordinary prompt read, replay, boss hint, and boss reveal. The current build actively uses ordinary prompt read and replay, and prepares boss hint / boss reveal on the same runtime speech path. If the backend supports speech-rate control, replay and future early boss hints may be slowed down, and if it does not, the system falls back to normal-speed speech rather than failing the run. Runtime support is treated in three operational buckets: unavailable (backend init fails), basic (speech works but rate/stop controls may be missing), and preferred (speech + stop + rate available). See `docs/audio.md` for the operational policy.
